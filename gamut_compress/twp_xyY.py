...
class Gamut_Conversion:
    ...
    def gamut_twp_xyY(self, sdr_RGB2020_signal):
        ...
        sdr2020_xyY =  torch.where( torch.isnan(sdr2020_xyY),  torch.zeros_like(sdr2020_xyY), sdr2020_xyY)  #TODO
        ...
        sdrx_cut = torch.where(rg_line_k * sdrx + rg_line_b < sdry, 
                            cross_rg[0], sdrx)
        sdry_cut = torch.where(rg_line_k * sdrx + rg_line_b < sdry, 
                            cross_rg[1], sdry)

        sdrx_cut1 = torch.where(rb_line_k * sdrx_cut + rb_line_b > sdry_cut, 
                            cross_rb[0], sdrx_cut)
        sdry_cut1 = torch.where(rb_line_k * sdrx_cut + rb_line_b > sdry_cut, 
                            cross_rb[1], sdry_cut)
        
        sdrx_cut2 = torch.where(gb_line_k * sdrx_cut1 + gb_line_b < sdry_cut1, 
                            cross_gb[0], sdrx_cut1)
        sdry_cut2 = torch.where(gb_line_k * sdrx_cut1 + gb_line_b < sdry_cut1, 
                            cross_gb[1], sdry_cut1)

        sdr709_xyY =  torch.cat((sdrx_cut2[..., None], sdry_cut2[..., None], sdr2020_xyY[:,:,2].clone()[..., None]), axis=-1)
...
if __name__ == "__main__":
    ...
    rgb709_example = torch.float32(torch.ones((540,960,3)) * 0.5)
    rgb2020_res = CG_conversion.CG_BT709_to_BT2020(rgb709_example)

    rgb2020_example = torch.float32(torch.ones((540,960,3)) * 0.5)
