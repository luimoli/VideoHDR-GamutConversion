...
class SdrConversion:
    def __init__(self, dark_point=(15, 18), skin_point=(63, 110), reference_point=(90.667, 203), sdr_ip=70,
                 high_light=(100, 400), a=120, b=0, alpha=1000, beta=0):
        """
        :param dark_point:
        :param skin_point:
        ...
        # self.cuda_available =  np.cuda.is_available()
        ...
        self.RGB2020_to_RGB709_matrix =  np.array([[1.660491, -0.587641, -0.072850],
    ...
    def tone_mapping(self, Y_hdr):
        ...
        Y_sdr =  np.empty_like(Y_hdr)
        Y_sdr =  np.where(Y_hdr <= self.hdr_dark, k0 * Y_hdr, Y_sdr)
        Y_sdr =  np.where((Y_hdr > self.hdr_dark) & (Y_hdr < self.hdr_ip), k1 * Y_hdr + b1, Y_sdr)
        Y_sdr =  np.where(Y_hdr >= self.hdr_ip, k2 *  np.log(Y_hdr / self.hdr_ip - k3) + k4, Y_sdr)
        return Y_sdr
    ...
    def hdr_to_sdr(self, image_rgb):
        ...
        image_xyz =  np.einsum('ic,hwc->hwi', self.RGB2020_to_XYZ_matrix, hdr_xRGB2020_linear)

        image_xyY = color_model.XYZ_to_xyY(image_xyz)
        # image_xyY = colour.XYZ_to_xyY(image_xyz)

        Y_sdr = self.tone_mapping(image_xyY[:, :, 2])
        sdr_xyY = image_xyY.copy()
        sdr_xyY[:, :, 2] = Y_sdr

        sdr_XYZ = color_model.xyY_to_XYZ(sdr_xyY)
        # sdr_XYZ = colour.xyY_to_XYZ(sdr_xyY)


        image_rgb_sdr =  np.einsum('ic,hwc->hwi', self.XYZ_to_RGB2020_matrix, sdr_XYZ)
        ...
        #--------------------2020 -> 709----------------------------------
        sdr_RGB2020_nolinear =  np.where( np.isnan(sdr_RGB2020_nolinear),  np.zeros_like(sdr_RGB2020_nolinear),
                                       sdr_RGB2020_nolinear)   

        # result_rgb_image_v0 =  np.einsum('ic,hwc->hwi', self.RGB2020_to_RGB709_matrix, sdr_RGB2020_nolinear)
